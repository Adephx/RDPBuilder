...
namespace RDPBuilder
{
    public partial class MainWindow : Window
    {
        ...
        private void LoadOrganizationalUnit(object sender, RoutedEventArgs e)
        {
            SelectOrganizationalUnit.SelectedItem = organizationalUnit;
        }
        ...
        public static Dictionary<string, string> ComputerNames
        {
            get
            {
                var sortedComputerNames = computerNames.OrderBy(kv => kv.Value).ToDictionary(kv => kv.Key, kv => $"{kv.Value} ({kv.Key})");
                return sortedComputerNames;
            }
            set
            {
                computerNames = value;
        ...
        static string? organizationalUnit;
        ...
        static void LoadData()
        {
            ...
            organizationalUnit = config?["organizationalUnit"]?.Value<string>();
        }
        ...
        static Dictionary<string, string> GetComputers()
        {
            ...
            using (DirectoryEntry entry = new("LDAP://" + domain))
            {
                ...
                foreach (SearchResult resEnt in mySearcher.FindAll())
                {
                    ...
                    {
                        computerNames.Add((string)resEnt.Properties["name"][0], (string)resEnt.Properties["description"][0]);
                    }
                }
            }
